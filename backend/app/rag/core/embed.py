...
class EmbeddingClient:
    ...
    def encode_texts(self, texts: List[str]) -> np.ndarray:
        ...
        embeddings = self._encode_request(texts)
        # Cache the embedding dimension from the first successful encode
    ...
    def dimension(self) -> int:
        ...
        Uses a small local table for common models; otherwise issues a
        single test request to discover the dimension and caches it.
        """
        if self._dimension is not None:
            return self._dimension
        # Check known mapping first
        if self.model_name in MODEL_DIMENSIONS:
            self._dimension = MODEL_DIMENSIONS[self.model_name]
            return self._dimension
        # Fall back to making a test call to infer the dimension
        emb = self._encode_request(["test"])
        if emb.size == 0:
            raise RuntimeError("Unable to determine embedding dimension from model response")
